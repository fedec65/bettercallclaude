...
### Fixed
- **Agents lost all MCP access on installs with unprefixed connector names** — every `tools:` frontmatter whitelisted MCP tools only under the Cowork plugin-scoped name (`mcp__plugin_bettercallclaude_<server>__<tool>`). On Cowork Desktop installs where the connectors register unprefixed (`mcp__<server>__<tool>` — user-verified on v4.11.5), the whitelist matched nothing: subagents ran with zero legal-database tools and every call failed with `Error: No such tool available: mcp__fedlex-sparql__get_article`, while direct session calls, generic agents, and inline commands all worked (same plugin, same release, 3 of 4 routes fine). All agent/skill/command `tools:` frontmatter now lists each MCP tool under **both** naming conventions (65 files regenerated via `scripts/generate-tool-frontmatter.js`); entries that match no registered tool are inert, so this is safe on every host — the same dual-prefix approach the privacy hook already uses for ollama. The generator now covers `agents/` (previously skills/commands only) and replaces the existing `tools:` block idempotently instead of inserting a duplicate. Whitelisted lists are otherwise unchanged. The two coordinator agents (`orchestrator`, `briefing`) also gain the `Task` tool, which their pipelines require to spawn specialist subagents — an omission dating back to before v4.11.5 (found in Devin review of this PR).
